...
// para aplicar una img bg-[url('/images/imagenapegar.png bg-black ${routerImageWeather[weatherInfo?.weather[0].icon]} ')]
  return (
    <main className={`min-h-screen text-white font-lato flex flex-col justify-center items-center px-4 relative ${routerImageWeather[resultSearch?.weather[0].icon]} bg-center bg-cover`}>
        
        <section className='felx flex-col  gap-4 sm:flex' >
        ...
        <div onClick={handleLanguage} className={ ` bg-cover   px-4 mb-4 w-8 h-8 ${lenguageIcon ? "bg-[url('/images/imgUs.png')]" : "bg-[url('/images/imgCol.png')]" }`} ></div>
        </section>

          <form onSubmit={handleSubmit} className="flex rounded-md overflow-hidden max-w-max mx-auto relative">
